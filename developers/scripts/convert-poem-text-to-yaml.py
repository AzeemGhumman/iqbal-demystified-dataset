...
def formatPoemId(poemId):
    # Split at '_', then format each portion and then join the 2 numbers together
    return '_'.join(['{:03d}'.format(int(num)) for num in poemId.split("_")])
...
for inputFilename in listInputFiles:
    ...
    outputFilename = os.path.join(os.path.dirname(outputFilename), poemId + ".yaml")
